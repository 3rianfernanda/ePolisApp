...
{
    public class PenutupanApprovalController : Controller
    {
        private readonly EpolisContext _context;

        public PenutupanApprovalController(EpolisContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> Index()
        {
            ...
            List<Tpenutupan> penutupan = _context.Tpenutupan.Where(m => m.STATUS == 19).ToList();
            ...
            if (inid != null)
                toperator = _context.Toperator.Where(x => x.APPROVERID == inid).ToList();
            else
        ...
        public async Task<IActionResult> Save(ViewModelPenutupan sreview)
        {
            ...
            {
                sreview.Tcatatan.TPENUTUPANID = sreview.Tcatatan.ID;
        ...
        public async Task<IActionResult> SaveCatatan(string catatan)
        {
            return PartialView("_Catatan", catatan);
        }
    }
}
